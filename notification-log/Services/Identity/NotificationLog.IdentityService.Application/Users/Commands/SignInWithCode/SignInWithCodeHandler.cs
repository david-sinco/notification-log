...
public sealed class SignInWithCodeHandler
{
    private readonly IUserRepository _users;
    private readonly IUserSecurity _security;

    public SignInWithCodeHandler(IUserRepository users, IUserSecurity security)
        => (_users, _security) = (users, security);

    public async Task<SignInAttempt> HandleAsync(SignInWithCodeCommand cmd, CancellationToken ct)
    {
