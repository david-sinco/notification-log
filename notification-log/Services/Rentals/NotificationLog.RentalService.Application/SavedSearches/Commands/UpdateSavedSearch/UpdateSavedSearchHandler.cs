...
public sealed class UpdateSavedSearchHandler
{
    private readonly ISavedSearchListRepository _searches;
    private readonly IIdentityReplica _identity;
    private readonly IUnitOfWork _uow;
    private readonly IValidator<UpdateSavedSearchCommand> _validator;

    public UpdateSavedSearchHandler(
        ISavedSearchListRepository searches,
        IIdentityReplica identity,
        IUnitOfWork uow,
        IValidator<UpdateSavedSearchCommand> validator)
        => (_searches, _identity, _uow, _validator) = (searches, identity, uow, validator);

    public async Task HandleAsync(UpdateSavedSearchCommand cmd, CancellationToken ct)
    {
        ...
        var searches = await _searches.LoadAsync(SavedSearchListId.For(cmd.UserId), ct)
            ?? throw new NotFoundException(nameof(SavedSearchList), cmd.UserId);

        searches.Update(cmd.SearchId, cmd.Name, cmd.Criteria.ToDomain(), cmd.Frequency);
