...
/// <summary>Thin wrapper over Users.Api's 11 endpoints (SPEC.md §4), one method each.</summary>
public sealed class UsersApiClient(HttpClient http)
{
    ...
    public Task<ApiResult<UserProfileDto>> GetProfileAsync(Guid id, CancellationToken ct) =>
        SendAsync<UserProfileDto>(http, HttpMethod.Get, $"/api/users/{id}", null, ct);
